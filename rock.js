...
function start(){
    ...
    ctx.drawImage(target,0,0,512,512,targetX - 50,targetY - 50,100,100);
    }
    catch(e) {
    }
...
function createThings(){
    var i;
    spriteMap[
        [0,120],[140,120],[270,120],[270,250],[400,120],[400,250],[530,250],[530,120]
    ]
    for(i = 0;i < 10;i++){
...
cur = new Thing(
    map[0],map[1],Math.floor((Math.random()*canvas.width)+1),
    Math.floor((Math.random()*canvas.height)+1),canvas   
)
 return cur
}
function checkKey(e){
...
function fixPageXY (e){
    if(e.page === null && e.clientX!== null){
        e.pageX = e.clientX+(html.scrollLeft||body&&body.scrollLeft||0)
        e.pageX-= html.clientLeft||0
        e.pageY = e.clientX+(html.scrollTop||body&&body.scrollTop||0)
...
function fire(){
    explosion.push(new Explosion(targetX,targetY,new Audio,))
    shots++
}
